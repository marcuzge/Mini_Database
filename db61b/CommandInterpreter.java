...
class CommandInterpreter {
    ...
    Table selectClause(String tableName) {
        _input.next("select");
        ArrayList<TableIterator> iter = new ArrayList<>();
        ...
        _input.next("from");
        Table tab = tableName();
        iter.add(tab.tableIterator());
        while (_input.nextIf(",")) {
            Table second = tableName();
            iter.add(second.tableIterator());
        }
        for (Column i : columns) {
            i.resolve(iter);
            colTitles.add(i.name());
        }
        List<Condition> conditionList = conditionClause(iter);
        Table newTable = new Table(tableName, colTitles);
        select(newTable, columns, iter, conditionList);
        return newTable;
    }
    ...
    Column columnSelector() {
        ...
        if (_input.peek().equals(".")) {
            _input.next();
            Table table = _database.get(table1);
            col2 = _input.next();
            newCol = new Column(table, col2);
        } else {
            newCol = new Column(null, table1);
        }
        return newCol;
    }
    ...
    List<Condition> conditionClause(List<TableIterator> iterators) {
        ...
        if (_input.nextIf("where")) {
            Condition c1 = condition(iterators);
            condList.add(c1);
        }
    ...
    Condition condition(List<TableIterator> iterators) {
        ...
        if (_input.nextIs(LITERAL)) {
            cond = new Condition(column1, relation, literal());
        } else {
            cond = new Condition(column1, relation, columnSelector(iterators));
        }
        return cond;
    }
    ...
    private void select(Table table, ArrayList<Column> columns,
        ...
        TableIterator iterator = iterators.get(0);
        if (iterators.size() == 0) {
            return;
        } else if (iterators.size() > 2) {
            throw error("Table size must be 1 or 2");
        } else if (conditions != null && iterators.size() == 1) {
            while (iterator.hasRow()) {
                Row curr = Row.make(columns);
        ...
        } else if (conditions != null && iterators.size() == 2) {
